...
One process runs: the **API**. It's configured by hand in the Render
dashboard (New → Web Service, native Node runtime), no Dockerfile involved.
...
Generate `JWT_SECRET` yourself (e.g. `openssl rand -base64 48`) — with no
blueprint, nothing generates it for you.

### 3. Create the service

In the Render dashboard: New → Web Service, point it at this repo, and set:

| Field | Value |
|---|---|
| Runtime | Node |
| Build Command | `npm ci --include=dev --fetch-retries=5 --fetch-retry-maxtimeout=60000 && npm run build -w @proofchain/shared && npm run build -w @proofchain/backend && npm prune --omit=dev` |
| Start Command | `node apps/backend/dist/main.js` |
| Health Check Path | `/health` |
| Plan | Free (sleeps after 15 min idle) |

`--include=dev` matters: `NODE_ENV=production` (set below) makes `npm ci`
skip devDependencies by default, but `nest build` needs `@nestjs/cli`, which
is one. `npm prune` at the end still strips them from what actually ships.
...
| `CORS_ORIGINS` | dashboard and capture origins, comma-separated |
| `PHOTO_S3_BUCKET` | bucket name for weigh-in photos |
| `PHOTO_S3_ENDPOINT` | the provider's S3 API URL (R2: `https://<account-id>.r2.cloudflarestorage.com`) |
| `PHOTO_S3_ACCESS_KEY_ID` | bucket access key |
| `PHOTO_S3_SECRET_ACCESS_KEY` | bucket secret key |

Photos go to an S3-compatible bucket because the instance's own disk is wiped
on every restart and deploy, which would leave events pointing at photos that
no longer exist. The region defaults to `auto`, which is right for R2; for
Supabase Storage or AWS, also add `PHOTO_S3_REGION` with the bucket's region.

`TRUST_PROXY=1` matters — without it, `req.ip` is Render's load balancer and
the login and ingest rate limits put every client in a single bucket.
...
Nothing above is Render-specific. Any host that can run a Node process
works — build and start it the same way the Render service's Build Command
and Start Command do:
